...
const AdminDashboard = () => {
  ...
  const { players, teams, auctionStatus, getPlayerStats } = useAuctionStore();
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-end pb-8">
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                <Badge
                  className={`mb-4 ${auctionStatus === 'running' ? 'bg-[#E50914]' : 'bg-[#333]'} text-white border-0`}
                >
                  {auctionStatus === 'running' && <span className="w-2 h-2 rounded-full bg-white animate-ping mr-2" />}
                  {auctionStatus.toUpperCase()}
                </Badge>
                <h1 className="font-heading text-5xl sm:text-6xl font-bold text-white tracking-wider">
                  ADMIN DASHBOARD
                </h1>
                <p className="text-gray-400 mt-2">KBN Premier League - Season 8</p>
              </div>
          ...
          {[
            ...
            >
              <motion.div whileHover={{ scale: 1.03, y: -5 }} className="card-hover">
                <Card className="bg-[#181818] border-[#333] hover:border-[#E50914]/50 transition-all">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-sm text-gray-500">{stat.label}</p>
                        <p className={`font-heading text-4xl font-bold mt-1 ${stat.color}`}>{stat.value}</p>
                        <p className="text-xs text-gray-600 mt-1">{stat.subValue}</p>
                      </div>
                      <motion.div 
                        className="p-3 rounded-lg bg-[#2F2F2F]"
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          >
            <Card className="bg-[#181818] border-[#333]">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 font-heading tracking-wider text-white">
              ...
              <CardContent className="space-y-3">
                {quickActions.map((action, index) => (
                  ...
                  >
                    <Button
                      variant="outline"
                      className="w-full justify-start h-auto py-4 px-4 border-[#333] bg-[#2F2F2F]/50 hover:bg-[#E50914]/10 hover:border-[#E50914] text-white transition-all"
                      onClick={() => navigate(action.path)}
                    >
                      <div className="p-2 rounded-lg bg-[#E50914] mr-3">
          ...
          >
            <Card className="bg-[#181818] border-[#333]">
              <CardHeader>
                ...
                <CardDescription className="text-gray-500">Player distribution by category</CardDescription>
              </CardHeader>
              <CardContent>
        ...
        >
          <Card className="bg-[#181818] border-[#333]">
            <CardHeader>
              ...
              <CardDescription className="text-gray-500">Current team standings and purse</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {teams.map((team, index) => (
                  ...
                  >
                    <div className="p-4 rounded-lg border border-[#333] bg-[#2F2F2F]/30 hover:border-[#E50914]/50 transition-all">
                      <div className="flex items-center gap-3 mb-3">
                        <motion.div
                          whileHover={{ rotate: 360 }}
                          transition={{ duration: 0.5 }}
                          className="w-12 h-12 rounded-lg flex items-center justify-center text-white font-heading font-bold text-xl"
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{team.players.length}/{team.maxPlayers} Players</p>
                        </div>
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-500">Purse Left</span>
                          <span className="font-semibold text-green-500">{formatCurrency(team.purse)}</span>
                        </div>
